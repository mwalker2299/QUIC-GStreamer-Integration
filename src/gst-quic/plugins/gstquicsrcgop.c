 ...
 * |[
 * gst-launch-1.0 quicsrcgop host=127.0.0.1 port=5000 caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, payload=(int)96, seqnum-base=(int)0\" ! rtpjitterbuffer latency={buffer_delay} ! rtph264depay ! h264parse ! queue ! decodebin ! videoconvert !  fakesink -v
 * ]|
...
//This function is not used, but must be defined for lsquic to work
static void
...
gst_quicsrcgop_start (GstBaseSrc * src)
{
  ...
  if (quicsrcgop->log_file[0] != '\0') {
    ...
    {    
      ...
      GST_WARNING_OBJECT (quicsrcgop, "Could not open logfile, errno: %d", errno);
    }
  } else {
    GST_WARNING_OBJECT (quicsrcgop, "No logfile provided, lsquic logs will not be created");
  }
