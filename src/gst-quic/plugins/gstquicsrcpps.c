...
gst_quicsrcpps_start (GstBaseSrc * src)
{
  ...
  if (quicsrcpps->log_file[0] != '\0') {
    ...
    lsquic_logger_init(&logger_if, s_log_fh, LLTS_HHMMSSUS);
      ...
      GST_WARNING_OBJECT (quicsrcpps, "Could not open logfile, errno: %d", errno);
    }
  } else {
    GST_WARNING_OBJECT (quicsrcpps, "No logfile provided, lsquic logs will not be created");
  }
