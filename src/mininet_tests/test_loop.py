...
def run_test(test_params, stream_server_command, stream_client_command, ct_command, timeout, protocol_name, log_path, log_level):
  ...
  if "UDP" in protocol_name:
    ...
    server_side_tshark_command = "tshark -d tcp.port==5000,rtp -T fields -e frame.time -e tcp.nxtseq -e rtp.seq -i s1-eth1 -o tcp.reassemble_out_of_order:TRUE > " + os.path.join(log_path, "server_side_timestamps.txt")
